...
const float gravityStrength = 2000;
...
void Game::Init(const char* title, SDL_Rect windowSize, int renderWidth, int renderHeight, bool fullscreen)
{
	...
	Registry::Instance()->transforms[square] = Transform_Component{ Vector2D(1200, 2000) };
	Registry::Instance()->rigidbodies[square] = Rigidbody_Component{ Vector2D(200, -400), Vector2D(0, gravityStrength) , 200 };
	...
	Registry::Instance()->transforms[square2] = Transform_Component{ Vector2D(1400, 600) };
